...
def get_exp_dist(lista, paired=False, method="weight-edge", distancia=None):
    ...
    if paired:
        ...
        joined = itertools.combinations(lista, r=2)
    ...
    for pair in joined:
        ...
        if method == "weight-edge":
            ...
            weights_1 = (pair[0] / distancia).flatten()
            weights_2 = (pair[1] / distancia).flatten()
...
def get_exp_measures(lista, paired=False, method="cutnorm"):
    ...
    if method == "cutnorm":
        ...
        for pair in tqdm(joined):
            ...
            nrmse = rmse / (
                max(np.max(cl1), np.max(cl2)) - min(np.min(cl1), np.min(cl2))
            )
            exp.append(nrmse)
        return exp
        ...
        for pair in tqdm(joined):
            ...
            nrmse = rmse / (
                max(np.max(deg1), np.max(deg2)) - min(np.min(deg1), np.min(deg2))
            )
            exp.append(nrmse)
        return exp
        ...
        if method == "cpc":
            ...
            for pair in joined:
                ...
                nrmse = rmse / (
                    max(np.max(weights_1), np.max(weights_2))
                    - min(np.min(weights_1), np.min(weights_2))
                )
                exp.append(nrmse)
            return exp
